...
__global__ void convolutionKernel(uint8_t* inPixels, int width, int height,
    ...
    int smemSize = blockDim.x + distanceToKernelCol * 2;
    int index_smem = smemSize * (distanceToKernelCol + threadIdx.y) + distanceToKernelCol + threadIdx.x;
    s_inPixels[index_smem] = inPixels[index];
    __syncthreads();

    if (threadIdx.y < distanceToKernelCol)
    {
        int index_gmem = max(0, row - distanceToKernelCol) * width + col;
        index_smem = smemSize * threadIdx.y + distanceToKernelCol + threadIdx.x;
        s_inPixels[index_smem] = inPixels[index_gmem];
    }
    if ((threadIdx.y > (blockDim.y - distanceToKernelCol - 1)))
    {
        int index_gmem = min(height - 1, row + distanceToKernelCol) * width + col;
        index_smem = smemSize * (threadIdx.y + distanceToKernelCol * 2) + distanceToKernelCol + threadIdx.x;
        s_inPixels[index_smem] = inPixels[index_gmem];
    }
    if (threadIdx.x < (distanceToKernelCol))
    {
        int index_gmem = row * width + max(0, col - distanceToKernelCol);
        index_smem = smemSize * (threadIdx.y + distanceToKernelCol) + threadIdx.x;
        s_inPixels[index_smem] = inPixels[index_gmem];
    }
    if ((threadIdx.x > (blockDim.x - distanceToKernelCol - 1)))
    {
        int index_gmem = row * width + min(width - 1, col + distanceToKernelCol);
        index_smem = smemSize * (threadIdx.y + distanceToKernelCol) + distanceToKernelCol * 2 + threadIdx.x;
        s_inPixels[index_smem] = inPixels[index_gmem];
    }
    if ((threadIdx.x > (blockDim.x - distanceToKernelCol - 1))
        && (threadIdx.y > (blockDim.y - distanceToKernelCol - 1)))
    {
        int row_check = min(height - 1, row + distanceToKernelCol);
        int col_check = min(width - 1, col + distanceToKernelCol);
        int index_gmem = row_check * width + col_check;
        index_smem = smemSize * (threadIdx.y + distanceToKernelCol * 2) + distanceToKernelCol * 2 + threadIdx.x;
        s_inPixels[index_smem] = inPixels[index_gmem];
    }
    if ((threadIdx.x < (distanceToKernelCol)) && (threadIdx.y < (distanceToKernelCol)))
    {
        int row_check = max(0, row - distanceToKernelCol);
        int col_check = max(0, col - distanceToKernelCol);
        int index_gmem = row_check * width + col_check;
        index_smem = smemSize * threadIdx.y + threadIdx.x;
        s_inPixels[index_smem] = inPixels[index_gmem];
    }
    if ((threadIdx.x < (distanceToKernelCol)) && (threadIdx.y > (blockDim.y - distanceToKernelCol - 1)))
    {
        int row_check = min(height - 1, row + distanceToKernelCol);
        int col_check = max(0, col - distanceToKernelCol);
        int index_gmem = row_check * width + col_check;
        index_smem = smemSize * (distanceToKernelCol * 2 + threadIdx.y) + threadIdx.x;
        s_inPixels[index_smem] = inPixels[index_gmem];
    }
    if ((threadIdx.x > (blockDim.x - distanceToKernelCol - 1)) && (threadIdx.y < (distanceToKernelCol)))
    {
        int row_check = max(0, row - distanceToKernelCol);
        int col_check = min(width - 1, col + distanceToKernelCol);
        int index_gmem = row_check * width + col_check;
        index_smem = smemSize * (threadIdx.y) + distanceToKernelCol * 2 + threadIdx.x;
        s_inPixels[index_smem] = inPixels[index_gmem];
    }
    __syncthreads();
    ...
    {
        int checkRow = (distanceToKernelCol + threadIdx.y) + rowCount;
        int checkColumn = distanceToKernelCol + threadIdx.x + columnCount;
        outPixel_x += s_inPixels[checkRow * (blockDim.x + (distanceToKernelCol * 2)) + checkColumn] * filter_x_Sobel[filterIndex];
        outPixel_y += s_inPixels[checkRow * (blockDim.x + (distanceToKernelCol * 2)) + checkColumn] * filter_y_Sobel[filterIndex];
    ...
    outPixels[index] = abs(outPixel_x) + abs(outPixel_y);
...
void convolutionByDevice(uint8_t* inPixels, int width, int height, int* outPixels, dim3 blockSize = dim3(1)) {
    ...
    timer.Start();
    int distanceToKernelCol = (filterWidth - 3) / 2 + 1;
    int smemSize = (blockSize.x + distanceToKernelCol * 2 + 1) * (blockSize.y + distanceToKernelCol * 2) * sizeof(uint8_t);
    dim3 gridSize((width - 1) / blockSize.x + 1, (height - 1) / blockSize.y + 1);
    convolutionKernel << <gridSize, blockSize,smemSize >> > (d_inPixels, width, height, d_filter_x_Sobel, d_filter_y_Sobel, filterWidth, d_outPixels);
    cudaError_t errSync = cudaGetLastError();
    cudaError_t errAsync = cudaDeviceSynchronize();
    if (errSync != cudaSuccess) {
        printf("Sync kernel error: %s\n", cudaGetErrorString(errSync));
